...
BEGIN
    htp.p('<script>');
        ...
        htp.prn('TR_END = "";');
        htp.p('const USUARIO = "'||doc.getusuario||'";');
...
BEGIN
    ...
    if nvl(prm_sessao, 'N/A') = 'N/A' then
        ws_cookie := owa_cookie.get('DOC_SESSION');
    ...
    BEGIN
        ...
        htp.p('<div class="main-conteudo" data-pergunta="'||ws_cd_pergunta||'">');
            ...
            htp.p('<div id="fundo-conteudo" class="fundo-conteudo '||lower(ws_tp_conteudo)||'">');
                ...
                if ws_classe = 'P' and nvl(doc.getusuario,'NOUSER') = 'NOUSER' then   -- se for documentação Privada é necessário logon no sistema 
                ...
                htp.p('<div class="bloco-direito-conteudo" id="bloco-direito-conteudo">');
                    htp.p('<div class="detalhe-conteudo2">');
                        ...
                        SELECT COUNT(*) INTO ws_count_rel
                          FROM DOC_PERGUNTAS
                         WHERE (ID_LIBERADO = 'S' OR doc.getNivel = 'A')
    ...
    BEGIN
        ...
        FOR A IN (SELECT A.NR_ORDEM, A.CD_PERGUNTA, B.PERGUNTA, (SELECT COUNT(*) FROM DOC_ESTRUTURA C WHERE C.CD_PERGUNTA_PAI = A.CD_PERGUNTA) as QT_FILHO 
                   ...
                   WHERE B.CD_PERGUNTA     = A.CD_PERGUNTA
                     AND (B.ID_LIBERADO = 'S' or doc.getNivel = 'A') 
...
begin
    ...
    if nvl(doc.getusuario,'NOUSER') = 'NOUSER' then 
...
procedure debug_cookie is
  c owa_cookie.cookie;
begin
  c := owa_cookie.get('DOC_SESSION');

  htp.p('<pre>');

  if c.num_vals = 0 then
    htp.p('RESULTADO: ORACLE NAO VE O COOKIE DOC_SESSION');
  else
    htp.p('RESULTADO: ORACLE VE O COOKIE DOC_SESSION');
...
begin
    ...
    if ws_usuario in ('DOC.ADMIN', 'DOC.GERENTE') then
        return 'A';
    else
        return 'N';
    end if;
